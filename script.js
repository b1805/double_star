...
let PHOTON_HEAD_SIZE = 0.70;
let PHOTON_TAIL_SIZE = 0.30;
let MAG_HEAD_SIZE = 3.00;
let MAG_TAIL_SIZE = 2.00;
...
function createPhotons2() {
  photons = new Array();
  var photonRadius = 15;
  var fractionOfAngle = -(((ANGLE-120) / 300) * (Math.PI * (5 / 3)) - (Math.PI * (4 / 3)));
  photons.push(new MultiMagPhoton(
      ORIGIN[0] + Math.cos(fractionOfAngle) * photonRadius,
      ORIGIN[1] + Math.sin(fractionOfAngle) * photonRadius,
      fractionOfAngle, 
...
function addPhoton() {
  ...
  var newPhoton = new MultiMagPhoton(
      ORIGIN[0] + Math.cos(fractionOfAngle) * photonRadius,
      ORIGIN[1] + Math.sin(fractionOfAngle) * photonRadius,
      fractionOfAngle,
      PRECISION,
  ...
  for (var mag = 0; mag < MAG.length; ++mag) {
    ...
    for (var i = 0; i < photons.length; ++i) {
      if (MAG[mag].magSquare.contains(photons[i].x, photons[i].y)) {
        ...
        drawCircle(viewerPos[0], viewerPos[1], MAG_HEAD_SIZE, photons[i].headColor);
        drawLine(photons[i].magEntry[mag][0], photons[i].magEntry[mag][1],
            viewerPos[0], viewerPos[1], photons[i].tailColor, MAG_TAIL_SIZE);
      }
    }
